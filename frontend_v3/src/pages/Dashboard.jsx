...
export default function Dashboard() {
  ...
  // =============================================
  // 📡 OPTIMIZED DATA FETCHING WITH AUTO-RETRY
  // =============================================
  useEffect(() => {
    let isMounted = true;
    ...
    // ✅ FAST STATS LOADING WITH BACKGROUND RETRY
    const loadStats = async () => {
      try {
        ...
        console.warn(`⚠️ Stats attempt ${statsRetryAttempt + 1} failed:`, err.message);
        
        if (statsRetryAttempt < MAX_RETRIES && isMounted) {
          statsRetryAttempt++;
          console.log(`🔄 Retrying stats (${statsRetryAttempt}/${MAX_RETRIES})...`);
          setTimeout(() => loadStats(), 1000 * statsRetryAttempt); // Progressive delay
        } else if (isMounted) {
          // Show fallback after all retries
    ...
    // Initial load with 1.5s timeout, then retry in background
    loadStats();
    
    // ✅ LOAD ISSUES (PARALLEL)
    api.get('/issues')
      .then(res => {
        if (!isMounted) return;
    ...
    <div className="container fade-in" style={{ position: 'relative', paddingBottom: '50px' }}>
      ...
      <div className="grid-2 main-grid" style={{ alignItems: 'start', gap: '25px' }}>
        ...
        <div className="right-column" style={{ display: 'grid', gap: '20px' }}>
          ...
          {/* STUDENT ACTIVITY / DEPARTMENT STATUS */}
          {user.role === 'student' ? (
      ...
      <style>{`
        ...
        @media (min-width: 480px) and (max-width: 767px) {
          ...
          .stat-card-label { font-size: 0.65rem !important; line-height: 1.2 !important; }
        ...
        @media (min-width: 375px) and (max-width: 479px) {
          ...
          .stat-card-label { font-size: 0.55rem !important; line-height: 1.1 !important; word-break: break-word !important; }
        ...
        @media (max-width: 374px) {
          ...
          .stat-card-label { font-size: 0.5rem !important; line-height: 1.1 !important; word-break: break-word !important; hyphens: auto !important; }
