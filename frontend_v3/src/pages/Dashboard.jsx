...
<span style={{ marginLeft: '6px', fontWeight: '700', color: '#1e293b', fontSize: '0.95rem' }}>{rating.toFixed(1)}</span>
</div>
);
};
return (
...
{/* --- TOP: NOTICE BAR --- */}
<div className="notice-bar" style={{ background: '#fff1f2', border: '1px solid #fecdd3', borderRadius: '12px', color: '#be123c', marginBottom: '20px', display: 'flex', alignItems: 'center', height: '45px', overflow: 'hidden' }}>
...
<button onClick={() => setShowAllNotices(true)} className="view-all-btn" style={{ background: 'transparent', border: 'none', color: '#9f1239', fontWeight: '700', fontSize: '0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
<List size={14} /> <span>View All</span>
</button>
</div>
</div>
{/* --- TOP: HERO BANNER --- */}
<div className="glass-panel hero-banner" style={{ background: 'linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%)', color: 'white', border: 'none', marginBottom: '2rem' }}>
<div className="hero-date" style={{ fontSize: '0.9rem', opacity: 0.8, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '5px' }}>
{new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
</div>
<h1 className="hero-title" style={{ fontSize: '2.5rem', fontWeight: '800', marginBottom: '0.5rem' }}>Welcome, {user.full_name.split(' ')[0]}! 👋</h1>
<p className="hero-desc" style={{ fontSize: '1.1rem', opacity: 0.9, maxWidth: '600px' }}>
{user.role === 'student' ? 'Track your complaints & view campus updates.' : 'Overview of maintenance performance & analytics.'}
</p>
<button onClick={() => setShowEscalation(true)} className="hero-btn" style={{ marginTop: '20px', background: 'white', color: '#4f46e5', border: 'none', padding: '10px 20px', borderRadius: '8px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
<Info size={18} /> <span>How Escalation Works</span>
</button>
</div>
{/* --- MIDDLE: STATS GRID (NEW PREMIUM 4-IN-ROW DESIGN) --- */}
<div 
...
>
<StatCard link="/issues" icon="📂" value={stats.total_issues} label="Total Issues" color="rgba(255, 255, 255, 0.9)" textColor="#1e293b" borderColor="#e2e8f0" />
<StatCard link="/issues" icon="⏳" value={stats.pending} label="Pending" color="#fffbeb" textColor="#b45309" borderColor="#fcd34d" />
<StatCard link="/issues" icon="✅" value={stats.resolved} label="Resolved" color="#f0fdf4" textColor="#15803d" borderColor="#86efac" />
{user.role === 'student' ? (
<StatCard link="/issues" icon="👤" value={stats.my_issues} label="My Reports" color="#eef2ff" textColor="#4338ca" borderColor="#a5b4fc" />
) : (
<StatCard link="/issues" icon="🔥" value={delayedCount} label="Delayed (>3 Days)" color="#fee2e2" textColor="#991b1b" borderColor="#fca5a5" />
)}
</div>
{/* --- BOTTOM: MAIN GRID --- */}
...
<h3 className="panel-title" style={{ fontSize: '1.1rem', fontWeight: 700, color: '#1e293b', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>🔥 Live Heatmap</h3>
<div className="heatmap-counter" style={{ fontSize: '0.75rem', color: '#64748b' }}>{heatmapIndex + 1} / 7</div>
</div>
<div className="heatmap-img-container" style={{ position: 'relative', height: '220px', background: '#f1f5f9', cursor: 'zoom-in' }} onClick={() => setZoomImage(HEATMAP_IMAGES[heatmapIndex])}>
...
<div className="zoom-badge" style={{ position: 'absolute', bottom: '10px', right: '10px', background: 'rgba(0,0,0,0.6)', color: 'white', padding: '4px 8px', borderRadius: '6px', fontSize: '0.7rem', display: 'flex', alignItems: 'center', gap: '4px' }}><Maximize2 size={12} /> <span className="zoom-text">Full Screen</span></div>
</div>
<div className="heatmap-dots" style={{ display: 'flex', justifyContent: 'center', gap: '6px', padding: '10px' }}>
...
<h3 className="panel-title" style={{ fontSize: '1.2rem', fontWeight: 700, color: '#1e293b', marginBottom: '1.5rem' }}>🚀 Quick Actions</h3>
<div className="quick-actions-grid" style={{ display: 'grid', gap: '10px' }}>
<Link to="/issues" className="btn-grad main-action-btn" style={{ textAlign: 'center', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
<span>{user.role === 'student' ? '➕ Report Issue' : '📋 Manage Issues'}</span>
</Link>
<div className="action-grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
<Link to="/contacts" className="btn-ghost action-btn" style={{ textAlign: 'center' }}><span className="action-icon">📞</span> <span className="action-text">Contacts</span></Link>
<Link to="/profile" className="btn-ghost action-btn" style={{ textAlign: 'center' }}><span className="action-icon">👤</span> <span className="action-text">Profile</span></Link>
{user.role !== 'student' && <Link to="/students" className="btn-ghost action-btn" style={{ textAlign: 'center' }}><span className="action-icon">🎓</span> <span className="action-text">Students</span></Link>}
{user.role !== 'student' && <Link to="/mess-rating" className="btn-ghost action-btn" style={{ textAlign: 'center', borderColor: '#f97316', color: '#ea580c' }}><span className="action-icon">🍔</span> <span className="action-text">Mess</span></Link>}
</div>
{/* ADMIN REPORTS */}
{user.role !== 'student' && (
<div className="admin-reports" style={{ marginTop: '15px', borderTop: '1px solid #e2e8f0', paddingTop: '15px' }}>
<div className="admin-label" style={{ fontSize: '0.75rem', fontWeight: '700', color: '#94a3b8', textTransform: 'uppercase', marginBottom: '10px' }}>Admin Reports</div>
<div className="report-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
<button onClick={() => setReportModal({ open: true, type: 'issues' })} className="btn-ghost report-btn" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px', borderColor: '#6366f1', color: '#6366f1' }}><FileText size={16} /> <span>Issues</span></button>
<button onClick={() => setReportModal({ open: true, type: 'mess' })} className="btn-ghost report-btn" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px', borderColor: '#ec4899', color: '#ec4899' }}><FileSpreadsheet size={16} /> <span>Mess</span></button>
</div>
</div>
)}
</div>
...
<h3 className="panel-title" style={{ fontSize: '1.2rem', fontWeight: 700, color: '#1e293b', marginBottom: '1.5rem' }}>📊 Campus Resolution Pulse</h3>
<div style={{ marginBottom: '15px' }}>
<div className="flex-between" style={{ fontSize: '0.9rem', marginBottom: '5px' }}>
<span className="progress-label" style={{ fontWeight: '600', color: '#475569' }}>Overall Progress</span>
<span className="progress-value" style={{ fontWeight: '700', color: '#4f46e5' }}>{resolutionRate}%</span>
</div>
...
<div className="status-metrics" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
<div className="metric-card" style={{ background: '#f8fafc', padding: '10px', borderRadius: '8px' }}>
<div className="metric-label" style={{ fontSize: '0.8rem', color: '#64748b' }}>Avg. Solve Time</div>
<div className="metric-value" style={{ fontSize: '1.2rem', fontWeight: '700', color: '#1e293b' }}>28 Hours</div>
</div>
<div className="metric-card" style={{ background: '#f8fafc', padding: '10px', borderRadius: '8px' }}>
<div className="metric-label" style={{ fontSize: '0.8rem', color: '#64748b' }}>Satisfaction</div>
<div className="metric-value" style={{ fontSize: '1.2rem', fontWeight: '700', color: '#d97706' }}>4.2 / 5.0</div>
</div>
</div>
</div>
</div>
...
{/* 2. Mess Rating (Student) / Staff Performance (Admin) */}
{user.role === 'student' ? (
<div className="glass-panel mess-rating-panel" style={{ background: 'linear-gradient(135deg, #fff7ed 0%, #ffedd5 100%)', border: '1px solid #fed7aa', textAlign: 'center', padding: '1.5rem' }}>
<div style={{ fontSize: '2.5rem', marginBottom: '10px' }}>🍔</div>
<h3 className="mess-title" style={{ fontSize: '1.2rem', fontWeight: '800', color: '#9a3412', marginBottom: '5px' }}>Mess Quality Rating</h3>
<Link to="/mess-rating" className="btn-grad mess-btn" style={{ background: 'linear-gradient(to right, #f97316, #ea580c)', display: 'inline-block', width: '100%', textDecoration: 'none', padding: '10px', fontSize: '0.9rem' }}>⭐ Rate Now</Link>
</div>
) : (
<div className="glass-panel staff-panel">
<h3 className="panel-title" style={{ fontSize: '1.2rem', fontWeight: 700, color: '#1e293b', marginBottom: '1.5rem' }}>👷 Staff Performance</h3>
<div className="staff-list" style={{ display: 'grid', gap: '12px' }}>
{WORKER_PERFORMANCE.map((w, i) => (
<div key={i} className="staff-card" style={{
padding: '12px',
background: '#f8fafc',
borderRadius: '10px',
...
<div className="staff-icon" style={{ background: '#e0e7ff', padding: '10px', borderRadius: '10px', color: '#4338ca' }}>{w.icon}</div>
<div className="staff-info">
<div className="staff-role" style={{ fontWeight: '700', color: '#1e293b', fontSize: '0.95rem', marginBottom: '2px' }}>{w.role}</div>
<div className="staff-tasks" style={{ fontSize: '0.75rem', color: '#64748b' }}>{w.tasks} Tasks</div>
</div>
</div>
...
{/* 3. Student Activity / Department Status */}
{user.role === 'student' ? (
<div className="glass-panel activity-panel">
<h3 className="panel-title" style={{ fontSize: '1.2rem', fontWeight: 700, color: '#1e293b', marginBottom: '1.5rem' }}>⚡ Your Activity</h3>
...
const truncatedTitle = displayTitle.length > 30 ? displayTitle.slice(0, 30) + '...' : displayTitle;
...
<div style={{ background: statusBg, padding: '6px', borderRadius: '50%', color: statusColor }}>
<Activity size={14}/>
</div>
<span className="activity-title" style={{ fontWeight: '700', fontSize: '0.85rem', color: '#1e293b' }}>
{truncatedTitle}
</span>
</div>
</div>
<div className="activity-tags" style={{ display: 'flex', gap: '12px', fontSize: '0.75rem', color: '#64748b', marginLeft: '30px' }}>
{issue.category && (
...
<div className="flex-between activity-footer" style={{ marginTop: '8px', marginLeft: '30px' }}>
...
<div className="no-activity" style={{textAlign: 'center', color: '#94a3b8', fontSize: '0.9rem', padding: '20px'}}>
No recent issues. <Link to="/issues" style={{ color: '#4f46e5', fontWeight: '600' }}>Report one now!</Link>
</div>
)}
</div>
</div>
...
<h3 className="panel-title" style={{ fontSize: '1.2rem', fontWeight: 700, color: '#1e293b', marginBottom: '1.5rem' }}>🏢 Department Status</h3>
<div className="department-list" style={{ display: 'grid', gap: '10px' }}>
<div className="flex-between dept-item" style={{ fontSize: '0.9rem' }}><span style={{ color: '#475569' }}>Academic Blocks</span><span className="dept-value" style={{ fontWeight: '700', color: '#1e293b' }}>12 Active</span></div>
...
<div className="flex-between dept-item" style={{ fontSize: '0.9rem', marginTop: '10px' }}><span style={{ color: '#475569' }}>Hostels</span><span className="dept-value" style={{ fontWeight: '700', color: '#1e293b' }}>45 Active</span></div>
...
/* ============================================
   PREMIUM STATS CARDS - 4-IN-ROW DESIGN
   ============================================ */
...
/* 1. LARGE DESKTOP (1920px+) - Premium Spacious Layout */
...
/* 2. DESKTOP (1440px - 1919px) - Standard Desktop */
@media (min-width: 1440px) and (max-width: 1919px) {
  .container { max-width: 1400px; margin: 0 auto; }
  .hero-title { font-size: 2.5rem !important; }
}

/* 3. LAPTOP (1024px - 1439px) - Laptop Standard */
@media (min-width: 1024px) and (max-width: 1439px) {
  .container { padding: 0 20px !important; }
  .hero-title { font-size: 2.2rem !important; }
}

/* 4. TABLET (768px - 1023px) - Tablet Portrait/Landscape */
@media (min-width: 768px) and (max-width: 1023px) {
  .container { padding: 0 16px !important; }
  .main-grid { grid-template-columns: 1fr !important; gap: 20px !important; }
  
  /* Stats - Still 4 in row on tablet */
  .stats-grid {
...
@media (min-width: 480px) and (max-width: 767px) {
  ...
  .staff-tasks { font-size: 0.65rem !important; }
...
/* 6. SMALL MOBILE (375px - 479px) - iPhone SE, Small Androids */
@media (min-width: 375px) and (max-width: 479px) {
  .container { padding: 0 8px !important; padding-bottom: 25px !important; }
  
  /* Stats Grid - 4 in ONE ROW (COMPACT) */
  .stats-grid {
    grid-template-columns: repeat(4, 1fr) !important;
    gap: 6px !important;
...
/* 7. MICRO MOBILE (<375px) - Very Small Phones, Mini Devices */
@media (max-width: 374px) {
  .container { padding: 0 6px !important; padding-bottom: 20px !important; }
  
  /* Stats Grid - 4 in ONE ROW (ULTRA COMPACT) */
  .stats-grid {
    grid-template-columns: repeat(4, 1fr) !important;
    gap: 4px !important;
